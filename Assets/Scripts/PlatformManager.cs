...
public class PlatformManager : MonoBehaviour
{
    ...
    [SerializeField]
    private float maxSeed = 12f;
    [SerializeField]
    private float acceleration = 0.1f;
    [SerializeField]
    private UnityEvent<Platform> onPlatformPased;
    ...
    public void InstantiatePlatform(int number)
    {
        for (int i = 0; i < number; i++)
        {
            ...
            onPlatformPased?.Invoke(newPlatform);
        }
    }
    private void Update()
    {
        if (isRunning)
        {
            ...
            speed = Mathf.Min(speed + acceleration * Time.deltaTime, maxSeed);
        }
    }
